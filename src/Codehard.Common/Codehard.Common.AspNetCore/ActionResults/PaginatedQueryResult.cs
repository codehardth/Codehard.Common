...
/// <summary>
/// Represents a paginated asynchronous query result for use as an IActionResult in ASP.NET Core MVC.
/// </summary>
/// <typeparam name="T">The type of the data returned in the paginated query result.</typeparam>
public sealed class PaginatedAsyncQueryResult<T> : IActionResult
{
    /// <summary>
    /// Gets an empty paginated query result with no data and total records set to 0.
    /// </summary>
    public static readonly PaginatedAsyncQueryResult<T> Empty = new(AsyncEnumerable.Empty<T>(), 0);

    /// <summary>
    /// Initializes a new instance of the <see cref="PaginatedAsyncQueryResult{T}"/> class.
    /// </summary>
    /// <param name="data">The paginated data.</param>
    /// <param name="totalRecords">The total number of records.</param>
    public PaginatedAsyncQueryResult(IAsyncEnumerable<T> data, int totalRecords)
    {
        this.Data = data;
        this.TotalRecords = totalRecords;
    }
