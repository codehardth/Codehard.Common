...
public class AsyncEffectExtensionTests
{
    ...
    {
        // Arrange
        var eff = liftEff(async () => await ValueTask.FromException<int>(new Exception("Something went crazy")));

        // Act
        var res =
            await eff.MapFailToInternalServerError(data: new SomeData(1, "2"))
                .RunAsync();
    ...
    public async Task WhenMapFailToInternalServerErrorWithMessageFunc_ShouldHaveHttpResultErrorWithCorrespondingMessage()
    {
        ...
        // Act
        var res =
            await eff.MapFailToInternalServerError(err => $"The error message is {err.Message}")
                .RunAsync();
